...
#Generate additional fields with information not considered for clustering
uv_features <- mvrnorm(3*n, c(0, 0), matrix(c(1, 0, 0, 1), 2,2) )

#Create data frame for samples
data <- data.frame( rbind( samples, samples2, samples3), uv_features );
colnames( data ) <- c('SampleId', 'ModeId', 'X', 'Y', 'U', 'V');
